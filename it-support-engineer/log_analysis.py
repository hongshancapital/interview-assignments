...
for item in valid_lines:
    ...
    temp1 = [{'deviceName': deviceName, 'processName': processName, 'description': description,
             'timeWindow': timeWindow}, processId]
    temp2= {'deviceName': deviceName, 'processName': processName, 'description': description,
             'timeWindow': timeWindow}
    log_list.append(temp1)
    event_list.append(temp2)

# 准备事件列表：对除了processID以外，完全一样的消息去重
event_list_distinct = []
for item in event_list:
    if item not in event_list_distinct:
        event_list_distinct.append(item)
# 准备计算列表：把同时段，相同事件的processID以列表形式添加到字典里，每个事件是一个列表元素。

result = []
for i in event_list_distinct:
    processId_list = []
    for j in log_list:
        if i == j[0]:
...
with open('py_output.txt','w') as f1:
    f1.write(json_str)
"""
