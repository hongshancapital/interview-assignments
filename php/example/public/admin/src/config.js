...
layui.define(['laytpl', 'layer', 'element', 'util'], function(exports){
  exports('setter', {
    ...
    apiUrl:window.location.protocol + '//' + window.location.host //真实接口调用域名 Author:极客兄弟
    ,imgUrl:window.location.protocol + '//' + window.location.host //图片存放地址
  ...
  layui.use(['table', 'admin'], function () {
    ...
    var headers = {};
    // headers[setter.request.tokenName] = layui.datsetter.request.tokenName;
    table.set({
      headers: {
        access_token: layui.data('babamuAdmin').access_token
      },
      response: {
    ...
    var refreshToken = function (access_token) {
      //请求成功后，写入 token
      layui.data(setter.tableName, {
        key: setter.request.tokenName
        , value: access_token
      });
      headers[setter.request.tokenName] = access_token;
      table.set({
        where: headers//table在地址中同时传递token
      });
    };
          ...
          if (access_token == null || access_token.trim().length === 0) {
            ...
            if (xhr.responseJSON.hasOwnProperty(setter.response.dataName)) {
              if (xhr.responseJSON[setter.response.dataName].hasOwnProperty(setter.request.tokenName)) {
                access_token = xhr.responseJSON[setter.response.dataName][setter.request.tokenName];
                if (access_token != null && access_token.trim().length !== 0) {
                  refreshToken(access_token);
                }
              }
