...
export class TransformInterceptor<T> implements NestInterceptor<T, Response<T>> {
  intercept(context: ExecutionContext, next: CallHandler): Observable<Response<T>> {
    return next.handle().pipe(map(data => {
      ...
      if (typeof data === 'string') throw new HttpException(data, 302);
      if (data.statusCode) throw new HttpException(data, data.statusCode);
      switch (request.method) {
        case 'GET':
          statusCode = HttpStatus.OK;
          message = data.message || '获取成功'; // 'GET ' + HttpMessage.OK;
          break;
        case 'POST':
          statusCode = HttpStatus.CREATED;
          message = data.message || '操作成功'; // 'POST ' + HttpMessage.CREATED;
          break;
        case 'PATCH':
          statusCode = HttpStatus.OK;
          message = data.message || '操作成功'; // 'PATCH ' + HttpMessage.OK;
          break;
        case 'PUT':
          statusCode = HttpStatus.OK;
          message = data.message || '操作成功'; // 'PUT ' + HttpMessage.OK;
          break;
        case 'DELETE':
          statusCode = HttpStatus.OK;
          message = data.message || '删除成功'; // 'DELETE ' + HttpMessage.OK;
          break;
        default:
