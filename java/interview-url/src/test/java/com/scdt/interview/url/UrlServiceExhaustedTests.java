...
@SpringBootTest
public class UrlServiceExhaustedTests {
    ...
    @Test
    void generateShortUrlWithShortUrlExhausted() {
        long requestCount = (long)Math.pow(62, 8);
        urlService.getId().addAndGet(requestCount);

        Assertions.assertThrows(ShortUrlExhaustedException.class, () -> {
            urlService.generateShortUrl(validLongUrl);
        });
    }
}
