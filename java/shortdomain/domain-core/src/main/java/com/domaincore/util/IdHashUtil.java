...
public class IdHashUtil {
    ...
    public IdHashUtil(String salt) {
        this(salt, 0);
    }

    public IdHashUtil(String salt, int minHashLength) {
        this(salt, minHashLength, DEFAULT_ALPHABET);
    }

    public IdHashUtil(String salt, int minHashLength, String alphabet) {
        ...
        seps = IdHashUtil.consistentShuffle(seps, this.salt);
        ...
        alphabet = IdHashUtil.consistentShuffle(alphabet, this.salt);
    ...
    public static void main(String[] args) {
        System.out.println(new IdHashUtil("", 6).encode(637376929632366597L));
    }
    ...
    private String _encode(long... numbers) {
        ...
        for (int i = 0; i < numbers.length; i++) {
            ...
            alphabet = IdHashUtil.consistentShuffle(alphabet, buffer.substring(0, alphabet.length()));
            final String last = IdHashUtil.hash(num, alphabet);
        ...
        while (ret_str.length() > this.minHashLength) {
            alphabet = IdHashUtil.consistentShuffle(alphabet, alphabet);
        ...
        if (hashArray.length > 0) {
            ...
            if (!hashBreakdown.isEmpty()) {
                ...
                for (final String aHashArray : hashArray) {
                    ...
                    alphabet = IdHashUtil.consistentShuffle(alphabet, buffer.substring(0, alphabet.length()));
                    ret.add(IdHashUtil.unhash(subHash, alphabet));
                }
            }
        }
