...
import com.assignment.common.exception.ServiceException;
...
public class DomainServiceImpl implements DomainService {
    ...
    public Result getLongUrl(String shortUrl) {
        logger.info("查询短链接对应长链接服务[getLongUrl],请求参数{}", shortUrl);
        if (StringUtils.isBlank(shortUrl)) {
            return Result.error(ErrorCode.REQ_PARAM_EMPTY.getCode(), "请求url不能为空");
        }
        try {
