...
class UrlControllerTest {
    ...
    void createSurl() throws Exception {
        MockHttpServletRequestBuilder url1 = MockMvcRequestBuilders.post("/").param("url", "http://sina.com");
    ...
    void getUrl() throws Exception {
        MockHttpServletRequestBuilder url1 = MockMvcRequestBuilders.post("/").param("url", "http://163.com");
        ResultActions performTrue = mockMvc.perform(url1);
        performTrue.andExpect(MockMvcResultMatchers.status().is(200));
        String newRes = performTrue.andReturn().getResponse().getContentAsString();
        JSONObject jsonObject = JSONUtil.parseObj(newRes);
        String surl = (String) ((JSONObject) jsonObject.get("data")).get("surl");

        MockHttpServletRequestBuilder mockHttpServletRequestBuilder = MockMvcRequestBuilders.get("/" + surl);
