...
class UnitTesting_AppListViewModel_Tests: XCTestCase {
    ...
    func test_AppListViewModel_updateUserCollectedApps() async throws {
        ...
        for _ in 0...999 {
            ...
            vm.appInfos[index].isCollected.toggle()
            
            vm.updateUserCollectedApps(appInfo: vm.appInfos[index])
            
            // 检测本地保存是否更新
            let collectedApps = TestDatasHelper.fetchCollectedApps()
            XCTAssertTrue(vm.appInfos[index].isCollected == collectedApps.contains(vm.appInfos[index].trackId))
        }
