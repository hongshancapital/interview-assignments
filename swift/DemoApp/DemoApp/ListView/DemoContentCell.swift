...
struct DemoContentCell: View {
    ...
    var body: some View {
        HStack(alignment: .center, spacing: 0) {
            ...
            /// ❤ 按钮
            Button {
                let isSelect = self.selecteState[String(describing: self.model?.id)]
                if isSelect == "" || isSelect == nil {
              ...
              VStack(spacing: 0) {
                  let selected = (self.selecteState[(String(describing: self.model?.id))] != nil && self.selecteState[(String(describing: self.model?.id))] != "")
                  Image(selected  ? "icon_heart_red" : "icon_heart_gray")
                      .scaleEffect(selected ? 1.1 : 1.0)
              }
              .foregroundColor(.white)
              .cornerRadius(10)
              .frame(width: 20, height: 20, alignment: .trailing)
              .animation(.interpolatingSpring(stiffness: 20, damping: 5), value: (self.selecteState[(String(describing: self.model?.id))] != nil && self.selecteState[(String(describing: self.model?.id))] != ""))
            }
            .frame(width: 20, height: 20, alignment: .trailing)
        }
