...
typealias LoadResult = Result<(result: [InfoModel], hasMore: Bool), Error>
...
struct ConfigManager {
    var pageIndex = 0
    private var dataSource = [InfoModel]()
    ...
    mutating func loadMore() async -> LoadResult{
        ...
        let reslut = self.dataSource[startIndex..<endIndex].map { info -> InfoModel in
            var value : InfoModel = info
