...
struct ListItemView: View {
    ...
    var onFavoriteTapped: ((ListItemModel) -> Void)? = nil
    var body: some View {
        HStack {
            KFImage.url(model.artworkUrl60)
            ...
            Spacer()
            Image(systemName: model.favoriteIconName)
                .foregroundColor(model.favoriteIconColor)
                .scaleEffect(model.favoriteIconScaleEffect)
                .onTapGesture {
                    onFavoriteTapped?(model)
                }
        }
...
struct ListItemView_Previews: PreviewProvider {
    static var previews: some View {
        ...
        VStack {
            ListItemView(model: model1)
            ListItemView(model: model2)
        }
    }
}
