...
class TestCase(unittest.TestCase):
    @data(*test_data)
    @unpack
    def test_run(self, id, method, url, expect, real, result):
        self.value = None
        print(f"--------value是：{self.value}")
        self.method = method
        print(f"--------method是：{self.method}")
        self.url = url
        print(f"--------url是：{self.url}")
        self.expect = expect
        print(f"------expect的类型是：{type(self.expect)}")
        print(f"------expect是：{self.expect}")
        ...
        ch = ConfigHttp(self.value, self.method, self.url)
        # print(f"-----------------ch是:{ch}")
        statusCode, get_result = ch.run()
        self.real = str(get_result)
        print(f"------real类型是：{type(self.real)}")
        print(f"------real是：{self.real}")
