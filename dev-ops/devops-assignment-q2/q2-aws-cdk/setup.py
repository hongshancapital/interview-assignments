import setuptools


with open('README.md') as fp:
    long_description = fp.read()


setuptools.setup(
    name='DevOps Assignment Q2 AWS CDK',
    version='0.0.1',

    description='AWS CDK Project for DevOps Assignment Q2',
    long_description=long_description,
    long_description_content_type='text/markdown',

    author='author',

    package_dir={'': 'q2_aws_cdk'},
    packages=setuptools.find_packages(where='q2_aws_cdk'),

    install_requires=[
        'aws-cdk.core==1.89.0',
        'aws-cdk.aws-cloudwatch==1.89.0',
        'aws-cdk.aws-cloudwatch-actions==1.89.0',
        'aws-cdk.aws-ec2==1.89.0',
        'aws-cdk.aws-ecs==1.89.0',
        'aws-cdk.aws-ecr==1.89.0',
        'aws-cdk.aws-iam==1.89.0',
        'aws-cdk.aws-logs==1.89.0',
        'aws-cdk.aws-sns==1.89.0',
    ],

    python_requires='>=3.6',

    classifiers=[
        'Development Status :: 4 - Beta',

        'Intended Audience :: Developers',

        'License :: OSI Approved :: Apache Software License',

        'Programming Language :: JavaScript',
        'Programming Language :: Python :: 3 :: Only',
        'Programming Language :: Python :: 3.6',
        'Programming Language :: Python :: 3.7',
        'Programming Language :: Python :: 3.8',

        'Topic :: Software Development :: Code Generators',
        'Topic :: Utilities',

        'Typing :: Typed',
    ],
)
