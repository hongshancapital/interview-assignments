...
const Swiper: React.FC<ISwiper> = (props: ISwiper) => {
  const { children = [], className, autoplay = false, autoplayInterval = 3000, transitionDuration = 300 } = props
  ...
  useEffect(() => {
    ...
    const indicatorList = swiperRef.current!.querySelectorAll<HTMLDivElement>('.swiper-widget__indicator-item');
    if (autoplay) {
      ...
      <div className="swiper-widget__indicator">
        {
          children.map((item, index) => {
            return (
              <div key={index} className="swiper-widget__indicator-item">
              </div>
            )
          })
        }
