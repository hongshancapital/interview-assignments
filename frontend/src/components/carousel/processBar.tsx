...
export const ProcessBar: FC<ProcessBarProps> = ({ total, nextPageHandler, processDuration }) => {
    ...
    const nextAnimation = useCallback((curIdx = 0) => {
        setAnimationStyle(animationStyle.map((_, idx) => idx === curIdx ? {
            width: '50px',
            transition: `${processDuration}s width`
        } : {
            width: '0px',
            transition: '0s'
        }
    ...
    }, [animationStyle, nextPageHandler, current, processDuration, total])
